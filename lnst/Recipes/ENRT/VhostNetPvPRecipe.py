...
from lnst.Recipe.ENRT.BasePvPRecipe import BasePvPTestConf, BasePvPRecipe
from lnst.Recipe.ENRT.BasePvPRecipe import VirtioDevice, VirtioType
...
class VhostNetPvPRecipe(BasePvPRecipe):
    ...
    def check_params(self):
        ...
        emulator_min, emulator_max = self.params.guet_emulatorpin_cpu.split('-')
    ...
    def host_forwarding_vm_configuration(self, host_conf, guest_conf):
        ...
        vhost_pids = host_conf.host.run("""ps --ppid 2 | grep "vhost-$(pidof qemu-kvm)" | awk '{if (length(pidstring) == 0) { pidstring=$1 } else { pidstring = sprintf("%s,%s", pidstring, $1) }}; END{ print pidstring }'""")
